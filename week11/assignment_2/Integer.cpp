...
Number *Integer::add(Number *num)
{
    ...
    else if (num->types() == COMPLEX)
    {
        ...
        cx->set_val(std::make_pair(this->value + cx->real(),cx->imag()));
        delete this;
        return cx;
    }
    else
...
Number *Integer::mul(Number *num)
{
    ...
    {
        Integer *it = dynamic_cast<Integer *>(num);
        this->set_val(this->value * it->val());
        delete it;
        return this;
    }
    else if (num->types() == FLOAT)
    ...
    {
        Complex *cx = dynamic_cast<Complex *>(num);
        cx->set_val(std::make_pair(this->value * cx->real(),this->value * cx->imag()));
        delete this;
        return cx;
    }
    else
